...
export function ReportForm({ defaultValues = {}, onSubmit, submitLabel = 'Save Report', isSubmitting = false, projects = [] }) {
  const {
    register,
    handleSubmit,
    formState: { errors }
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 rounded-3xl border border-slate-800 bg-slate-900/70 p-6 shadow-xl">
      <div className="grid gap-4 md:grid-cols-2">
        <div>
          ...
          {projects && projects.length > 0 ? (
            <select
              ...
              className="w-full rounded-2xl border border-slate-700 bg-slate-950/80 px-4 py-3"
            >
        ...
        <div>
          ...
          <input
            type="number"
            {...register('hours_worked', { required: 'Hours are required', min: 1 })}
            className="w-full rounded-2xl border border-slate-700 bg-slate-950/80 px-4 py-3"
            placeholder="40"
          />
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div>
          ...
          <input
            type="date"
            {...register('week_end', { required: 'Week end is required' })}
            className="w-full rounded-2xl border border-slate-700 bg-slate-950/80 px-4 py-3"
          />
        </div>
      </div>
      ...
      <div>
        ...
        <textarea
          {...register('tasks_completed', { required: 'This field is required', minLength: 5 })}
          className="min-h-24 w-full rounded-2xl border border-slate-700 bg-slate-950/80 px-4 py-3"
        />
      ...
      <div>
        ...
        <textarea
          {...register('tasks_planned', { required: 'This field is required', minLength: 5 })}
          className="min-h-24 w-full rounded-2xl border border-slate-700 bg-slate-950/80 px-4 py-3"
        />
        {errors.tasks_planned && <p className="mt-1 text-sm text-rose-400">{errors.tasks_planned.message}</p>}
      </div>
